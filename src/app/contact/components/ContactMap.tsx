    ...
    <div className="reveal mt-16">
      ...
      <div className="w-full h-[380px] md:h-[460px] rounded-[2rem] overflow-hidden relative border border-white/60 shadow-xl bg-primary/5">
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 text-center px-6 z-10">
          ...
          <a href="https://maps.google.com/?q=Koregaon+Park+Annexe+Pune" target="_blank" rel="noopener noreferrer" className="px-6 py-3 bg-primary text-cream rounded-full text-sm font-semibold hover:bg-accent transition-all">
            Open in Google Maps
          </a>
        </div>
        <div className="absolute inset-0 opacity-5 pointer-events-none" style={{ backgroundImage: 'linear-gradient(var(--color-primary) 1px, transparent 1px), linear-gradient(90deg, var(--color-primary) 1px, transparent 1px)', backgroundSize: '40px 40px' }} />
      </div>
    </div>
  );
}