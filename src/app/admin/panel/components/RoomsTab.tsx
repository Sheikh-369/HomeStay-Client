...
import { IRoomData } from '@/src/lib/store/room/room-slice-type';
import { createRoom, deleteRoom, fetchRooms, updateRoom } from '@/src/lib/store/room/room-slice';
// Import your types and thunks

// Keeping your original UI types for props (though rooms will now come from Redux)
interface RoomsTabProps {
  rooms: any[]; // We will use the data from Redux instead
...
export default function RoomsTab({}: RoomsTabProps) {
  const dispatch = useAppDispatch();
  
  // 1. Grab dynamic data from Redux
  const { rooms: rawRooms, status } = useAppSelector((state) => state.roomSlice);

  // 2. Map API data to UI structure (handling naming differences)
  const rooms = rawRooms.map((r: IRoomData) => ({
    id: r.roomNumber, // UI expects id
    _dbId: r.id,      // Keep internal DB id for API calls
    floor: r.floor,
    type: r.roomType,
    capacity: r.capacity,
    occupied: r.status === 'Occupied', // Convert string to boolean for UI logic
    amenities: r.features || [],       // Map features to amenities
    rent: r.rent
  }));
  ...
  const [addOpen, setAddOpen] = useState(false);
  const [newForm, setNewForm] = useState<Partial<any>>({ amenities: [] });
  ...
  const openEdit = (r: any) => { 
    setEditRoom(r); 
    setEditForm({ ...r, amenities: [...r.amenities] }); 
  };

  const toggleAmenity = (amenity: string, form: any, setForm: (f: any) => void) => {
    const current = form.amenities || [];
  ...
  const handleCreate = async () => {
    const payload: IRoomData = {
      roomNumber: newForm.id || '',
      floor: newForm.floor || '',
      roomType: newForm.type || '',
      capacity: newForm.capacity || 1,
      rent: newForm.rent || '',
      features: newForm.amenities || [],
      status: newForm.occupied ? "Occupied" : "Vacant"
    };
    await dispatch(createRoom(payload));
    setAddOpen(false);
    setNewForm({ amenities: [] });
  };
  ...
  const handleUpdate = async () => {
    if (!editForm) return;
    const payload: Partial<IRoomData> = {
      roomNumber: editForm.id,
      floor: editForm.floor,
      roomType: editForm.type,
      capacity: editForm.capacity,
      rent: editForm.rent,
      features: editForm.amenities,
      status: editForm.occupied ? "Occupied" : "Vacant"
    };
    await dispatch(updateRoom(editForm._dbId, payload));
    setEditRoom(null);
    setEditForm(null);
  };
  ...
  const handleDelete = async () => {
    if (deleteTarget) {
      await dispatch(deleteRoom(deleteTarget._dbId));
      dispatch(fetchRooms()); // Refresh list after delete
          ...
          <div key={room.id} className="bg-cream rounded-2xl border border-primary/8 p-5 card-lift">
            ...
            <div className="mt-4 pt-3 border-t border-primary/8 flex items-center justify-between">
              <p className="font-display font-bold text-primary">
                Rs.<span className="text-accent"> {room.rent} </span>/mo
              </p>
              <div className="flex items-center gap-3">
                <button onClick={() => setViewRoom(room)} className="text-xs font-semibold text-accent hover:text-accent-light transition-colors">View</button>
                <button onClick={() => openEdit(room)} className="text-xs font-semibold text-primary/50 hover:text-primary transition-colors">Edit</button>
        ...
        {viewRoom && (
          <div className="space-y-4">
            ...
            <div>
              <p className="text-xs text-primary/40 font-semibold uppercase tracking-wider mb-2">Amenities</p>
              <div className="flex flex-wrap gap-1.5">
                {viewRoom.amenities.map((a: string) => (
                  <span key={a} className="px-2.5 py-1 rounded-lg bg-primary/5 text-xs font-medium text-primary/70">{a}</span>
                ))}
              </div>
            </div>
      ...
      <Modal isOpen={!!editRoom} onClose={() => { setEditRoom(null); setEditForm(null); }} title="Edit Room" size="md">
        {editForm && (
          <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-2 mt-1">
                {AMENITY_OPTIONS.map((a) => (
                  <button
                    key={a}
                    type="button"
                    onClick={() => toggleAmenity(a, editForm, setEditForm)}
                    className={`px-3 py-1 rounded-lg text-xs font-semibold transition-all ${editForm.amenities.includes(a) ? 'bg-primary text-cream' : 'bg-primary/5 text-primary/60 hover:bg-primary/10'}`}
                  >
                    {a}
                  </button>
            ...
            <div className="flex gap-2 pt-2">
              <button onClick={handleUpdate} className="flex-1 py-2.5 px-4 bg-primary text-cream text-sm font-semibold rounded-xl hover:bg-primary/85 transition-all">
                Save Changes
              </button>
              <button onClick={() => { setEditRoom(null); setEditForm(null); }} className="flex-1 py-2.5 px-4 border border-primary/15 text-primary/60 text-sm font-semibold rounded-xl hover:border-primary/30 transition-all">
                Cancel
      ...
      <Modal isOpen={!!deleteTarget} onClose={() => setDeleteTarget(null)} title="Delete Room" size="sm">
        {deleteTarget && (
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              <button onClick={handleDelete} className="flex-1 py-2.5 px-4 bg-red-500 text-white text-sm font-semibold rounded-xl hover:bg-red-600 transition-all">
                Delete
              </button>
              <button onClick={() => setDeleteTarget(null)} className="flex-1 py-2.5 px-4 border border-primary/15 text-primary/60 text-sm font-semibold rounded-xl hover:border-primary/30 transition-all">
                Keep
